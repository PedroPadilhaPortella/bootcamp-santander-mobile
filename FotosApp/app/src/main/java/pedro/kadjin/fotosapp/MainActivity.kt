        ...
        pick_button.setOnClickListener {
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if(checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED
                    ...
                    requestPermissions(permissions, PERMISSION_CODE_IMAGE_PICK)
                } else {
                    pickImageFromGalery()
                }
            } else {
        ...
        when(requestCode) {
            PERMISSION_CODE_IMAGE_PICK -> {
                if(grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(this, "Permissão de Acesso a Galeria Negada", Toast.LENGTH_SHORT).show()
                }
            }
            PERMISSION_CODE_CAMERA -> {
                if(grantResults.size > 1 && grantResults[0] == PackageManager.PERMISSION_GRANTED && grantResults[1] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(this, "Permissão de Acesso a Camera Negada", Toast.LENGTH_SHORT).show()
                }
            }
