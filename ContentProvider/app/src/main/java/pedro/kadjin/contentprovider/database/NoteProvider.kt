...
import pedro.kadjin.contentprovider.database.NotesDatabaseHelper.Companion.TABLE_NAME
...
class NoteProvider : ContentProvider() {
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        if(mUriMatcher.match(uri) == NOTES) {
            ...
            throw UnsupportedSchemeException("URI invalida para executar a insercao")
        }
    }
    ...
    override fun query(
        ...
        return when {
            mUriMatcher.match(uri) == NOTES -> {
                val db: SQLiteDatabase = dbHelper.writableDatabase
                val cursor: Cursor = db.query(TABLE_NAME, projection, selection, selectionArgs, null, null, sortOrder)
                cursor.setNotificationUri((context as Context).contentResolver, uri)
                cursor
            }
            mUriMatcher.match(uri) == NOTES_BY_ID -> {
                val db: SQLiteDatabase = dbHelper.writableDatabase
                val cursor: Cursor = db.query(TABLE_NAME, projection, "$_ID == ?", arrayOf(uri.lastPathSegment), null, null,  sortOrder)
                cursor.setNotificationUri((context as Context).contentResolver, uri)
                cursor
            }
            else -> {
    ...
    companion object {
        const val AUTHORITY = "pedro.kadjin.provider"
