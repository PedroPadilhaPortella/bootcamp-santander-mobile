...
import pedro.kadjin.contentprovider.database.NoteProvider.Companion.URI_NOTES
import pedro.kadjin.contentprovider.database.NotesDatabaseHelper.Companion.DESCRIPTION_FIELD
import pedro.kadjin.contentprovider.database.NotesDatabaseHelper.Companion.TITLE_FIELD
...
class NotesDetailFragment : DialogFragment(), DialogInterface.OnClickListener {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        if(arguments != null && arguments?.getLong(EXTRA_ID) != 0L) {
            ...
            if(cursor?.moveToNext() as Boolean) {
                newNote = false
                noteEditTitle.setText(cursor.getString(cursor.getColumnIndex(TITLE_FIELD)))
                noteEditDescription.setText(cursor.getString(cursor.getColumnIndex(DESCRIPTION_FIELD)))
            }
            cursor.close()
        }
    ...
    override fun onClick(dialog: DialogInterface?, which: Int) {
        ...
        values.put(TITLE_FIELD, noteEditTitle.text.toString())
        values.put(DESCRIPTION_FIELD, noteEditDescription.text.toString())
